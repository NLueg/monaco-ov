...
export function createVSCodeApi(servicesProvider: Services.Provider): typeof vscode {
    ...
    const workspace: typeof vscode.workspace = {
        ...
        get onDidChangeConfiguration(): typeof vscode.workspace.onDidChangeConfiguration {
            const services = servicesProvider();
            return (listener: (e: vscode.ConfigurationChangeEvent) => any, thisArgs?: any, disposables?: Disposable[]): Disposable => {
                if (services.workspace.configurations) {
                    return services.workspace.configurations.onDidChangeConfiguration(({ affectsConfiguration }) => {
                        const l: (e: vscode.ConfigurationChangeEvent) => any = listener.bind(thisArgs);
                        l({
                            affectsConfiguration: affectsConfiguration
                        });
                    });
                } else {
                    return { dispose: console.error };
                }
            }
            return Event.None;
        },
